...
const result = countMainCourseItems(menu);
...
function getCheapestMenuItem(menu) {
  // Your code here
  const lowestPrice = Math.min(menu.map((item) => item.price));
  return menu.find((item) => item.price == lowestPrice);
}
console.log(result);
...
function getMenuItemsByIngredient(menu, ingredient) {
  // Your code here
  return menu.filter((item) => item.ingredients.includes(ingredient));
}
console.log(result);
